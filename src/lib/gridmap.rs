...
impl GridMap {
	...
	pub fn link_cells(&self, lhv : usize, rhv : usize) -> () {
		...
		self.map.borrow().get(&lhv).unwrap().link_north(self.map.borrow().get(&rhv).unwrap());
		self.map.borrow().get(&rhv).unwrap().link_south(self.map.borrow().get(&lhv).unwrap());
	}
}
...
mod tests {
	...
	pub fn link_cells() {
		...
		assert_eq!(grid.map.borrow().get(&2).unwrap().get_south().unwrap(), 1);
	}
}